...
def close_position():
    try:
        position = bybit_api.position_my_get(symbol=SYMBOL).result()[0]['result'][0]
        if position['size'] > 0:
            bybit_api.Order.Order_closePosition(symbol=SYMBOL).result()
...
def buy_position():
    try:
        symbol_info = bybit_api.Symbol.Symbol_get(symbol=SYMBOL).result()[0]['result'][0]
        curr_price = symbol_info['last_price']
        qty = round((ACCOUNT_BALANCE * LEVERAGE * 0.7) / curr_price, 1)
        bybit_api.Order.Order_new(side='Buy', symbol=SYMBOL, order_type='Market', qty=qty).result()
        print('Buy position!')
    except Exception as e:
        print(e)
        time.sleep(10)
...
def main():
    global current_pos, current_price
    while True:
        tickers = bybit_api.Symbol.Symbol_get(symbol=SYMBOL).result()[0]['result'][0]['last_price']
        last_price = float(tickers)
        if current_pos == '':
            if check_candle():
                # 买入70%的仓位
